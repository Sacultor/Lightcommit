# ERC-8004 Trustless Agent 系统

## 概述

本项目实现了基于 ERC-8004 标准的去中心化 GitHub 贡献评估与链上凭证系统，通过智能合约自动化评估开发者贡献并铸造不可篡改的链上声誉凭证。

## 核心特性

### ✅ 已实现

- **身份注册表**：GitHub 用户与钱包地址绑定
- **声誉注册表**：EIP-712 签名的链上评分反馈系统
- **验证注册表**：基于阈值的自动 NFT 铸造决策
- **ERC-721 凭证**：不可篡改的贡献证明 NFT
- **多维度评分**：Commit Message、代码规模、文件影响等
- **IPFS 集成**：元数据去中心化存储
- **事件驱动**：实时链上事件监听

### 🔄 进行中

- Chainlink Functions 集成
- 批量处理优化
- 前端 UI 组件
- 移动端支持

### 📋 计划中

- DAO 治理模块
- 多链部署（Polygon、Arbitrum）
- zk 隐私证明
- EAS 集成

## 快速开始

### 1. 安装依赖
```bash
pnpm install
```

### 2. 启动本地链
```bash
cd hardhat
npx hardhat node
```

### 3. 部署合约
```bash
npx hardhat run scripts/deploy-erc8004.ts --network localhost
```

### 4. 配置前端
```bash
cd frontend
cp .env.erc8004.example .env.local
```

编辑 `.env.local`，填入部署的合约地址。

### 5. 启动前端
```bash
pnpm dev
```

访问 http://localhost:3000

## 系统架构

```
┌─────────────────┐
│  GitHub Webhook │
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│  评分服务 (API) │
│  - 多维度分析   │
│  - EIP-712签名  │
└────────┬────────┘
         │
         ▼
┌─────────────────────────────┐
│  ReputationRegistry (链上)  │
│  - 验证签名                 │
│  - 存储评分哈希             │
└────────┬────────────────────┘
         │
         ▼
┌─────────────────────────────┐
│  ValidationRegistry (链上)  │
│  - 阈值判断 (>= 80分)       │
│  - 自动触发铸造             │
└────────┬────────────────────┘
         │
         ▼
┌─────────────────┐
│  CommitNFT      │
│  (ERC-721)      │
└─────────────────┘
```

## 合约地址（本地开发）

部署后会自动显示，示例：
```
AgentIdentityRegistry: 0x5FbDB2315678afecb367f032d93F642f64180aa3
ReputationRegistry: 0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512
CommitNFT: 0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0
ValidationRegistry: 0xCf7Ed3AccA5a467e9e704C703E8D87F634fB0Fc9
```

## 核心流程

### 1. 注册身份
```typescript
await identityRegistry.registerAgent(
  "github-username",
  "ipfs://QmAgentCard"
);
```

### 2. 提交评分
```typescript
const { feedback, signature, metadataURI } = await fetch(
  `/api/contributions/${id}/sign`
).then(r => r.json());

await reputationRegistry.submitFeedback(
  feedback.contributor,
  feedback.repo,
  feedback.commitSha,
  feedback.score,
  feedback.feedbackHash,
  metadataURI,
  signature
);
```

### 3. 请求验证
```typescript
await validationRegistry.requestValidation(
  repo,
  commitSha,
  contributor,
  metadataURI
);
```

如果评分 >= 80，自动铸造 NFT。

## 评分维度

| 维度 | 权重 | 说明 |
|------|------|------|
| Commit Message | 25% | Conventional Commits 规范 |
| 代码规模 | 20% | additions + deletions |
| 文件影响 | 20% | 测试覆盖、文档等 |
| 合并信号 | 15% | 是否已合并 |
| 元数据完整性 | 20% | Issue 链接、描述等 |

## 文档

- **[部署指南](./DEPLOYMENT_GUIDE.md)** - 详细部署步骤
- **[实现文档](./ERC8004_IMPLEMENTATION.md)** - 技术实现细节
- **[使用示例](./ERC8004_USAGE_EXAMPLES.md)** - 代码示例
- **[API 参考](./API_REFERENCE.md)** - API 文档

## 测试

### 运行单元测试
```bash
cd hardhat
npx hardhat test
```

### 运行特定测试
```bash
npx hardhat test test/ERC8004.test.ts
```

### 查看覆盖率
```bash
npx hardhat coverage
```

## 安全特性

- ✅ EIP-712 签名验证
- ✅ 防重放攻击（nonce + timestamp）
- ✅ 角色权限控制
- ✅ 紧急暂停机制
- ✅ Commit 去重检查
- ✅ ReentrancyGuard 保护

## 性能优化

- **最小化链上存储**：仅存哈希和指针
- **事件驱动**：减少状态读取
- **批量处理**：支持批量评分（TODO）
- **Layer2 友好**：可部署到 Optimism/Arbitrum

## 常见问题

### Q: 评分阈值可以调整吗？
A: 可以，管理员可调用 `ValidationRegistry.setMintThreshold(newThreshold)`

### Q: IPFS 上传失败怎么办？
A: 检查 Pinata/Web3.Storage 配置，或使用模拟模式（开发环境）

### Q: 如何添加新的评分维度？
A: 修改 `frontend/src/lib/services/scoring.service.ts`

### Q: 支持哪些区块链？
A: 任何 EVM 兼容链（Ethereum、Polygon、Arbitrum、Optimism 等）

## 贡献指南

欢迎贡献！请遵循：
1. Fork 项目
2. 创建特性分支
3. 提交 PR
4. 确保测试通过

## 许可证

MIT License

## 联系方式

- Email: 2313072@mail.nankai.edu.cn
- GitHub: https://github.com/Sacultor/Lightcommit

## 致谢

- ERC-8004 标准：https://8004.org
- OpenZeppelin 合约库
- Hardhat 开发框架
- Ethers.js

